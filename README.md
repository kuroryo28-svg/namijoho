...
前日までに確定している生値を2つ出すだけの項目。**推奨には使わない。**

    前日終値位置   (前日終値 - 前日安値) / (前日高値 - 前日安値)   0=安値引け
    前3日 純変化   (前日終値 - 3営業日前の始値) / ATR14           符号つき

出どころは候補ログ1年ぶんの観測。日次損益 上位20日とその他286日を比べると、

| | 上位20日 | その他 |
|---|---|---|
| 前日終値位置 | 0.412 | 0.596 |
| 前3日 純変化 | +0.207 ATR | +3.379 ATR |

どちらも前半・後半の両方で同じ向きに出た。大きく獲れる日は
「上がった後」ではなく「溜まった後」に来ている、という形。

守ってること:

- **合成しない。** 2つを1つの点数にまとめない。まとめると中で何が効いて
  いるか分からなくなる
- **ラベルにしない。** 生値と百分位だけ出す。「高い/低い」は見る側が決める
- **推奨に入れない。** `select_board()` の引数にしない。SEL-v1 は無改造
- 当日以降の価格を使わない。前日の終値で確定する

判定はまだしない。毎日出して台帳に残し、当たるかを見る。
**日々の結果を見て閾値をいじり始めた時点で、記録全体が in-sample になる。**
...
`coil.py`
```
COIL_VERSION="coil-v0"  ATR_WIN=14  NET_DAYS=3  PCT_WIN=250
```
...
coil.py             溜まり（前日終値位置・前3日純変化）。表示のみ
review.py           予報と実測の突合
